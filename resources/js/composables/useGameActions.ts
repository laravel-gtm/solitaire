...
import AutoCompleteController from '@/actions/App/Http/Controllers/Solitaire/AutoCompleteController';
...
export function useGameActions(gameId: MaybeRef<string>) {
    ...
    async function autoComplete(): Promise<Game | null> {
        ...
        try {
            const response = await postJson<GameResponse>(AutoCompleteController.url(toValue(gameId)));
