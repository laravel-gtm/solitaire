...
import UndoMoveController from '@/actions/App/Http/Controllers/Solitaire/UndoMoveController';
...
export function useGameActions(gameId: MaybeRef<string>) {
    ...
    async function undoMove(): Promise<Game | null> {
        ...
        try {
            const response = await postJson<GameResponse>(UndoMoveController.url(toValue(gameId)));
